...
namespace ToolsForHaul
{
    ...
    {
        public static void MakeInteractionMote(
            Pawn initiator,
            Pawn recipient,
            ThingDef interactionMote,
            Texture2D symbol)
        {
        ...
        {
            if (!loc.ShouldSpawnMotesAt() || MoteCounterTFH.Saturated)
            {
                return;
            }
        ...
        {
            if (!loc.ShouldSpawnMotesAt() || MoteCounterTFH.SaturatedLowPriority)
            {
                return;
            }
        ...
        public static void PlaceTireTrack(Vector3 loc, float rot, Vector3 pos)
        {
                if (loc.ShouldSpawnMotesAt() && !MoteCounterTFH.SaturatedLowPriority)
                {
        ...
        {
            if (!loc.ToIntVec3().ShouldSpawnMotesAt() || MoteCounterTFH.SaturatedLowPriority)
            {
                return;
            }
        ...
        public static void ThrowFireGlow(IntVec3 c, float size)
        {
            ...
            if (!vector.ShouldSpawnMotesAt() || MoteCounterTFH.SaturatedLowPriority)
            {
                return;
            }
        ...
        {
            if (!thrower.Position.ShouldSpawnMotesAt() || MoteCounterTFH.Saturated)
            {
                return;
            }
            ...
            Vector3 vector = targetCell.ToVector3Shifted()
                             + Vector3Utility.RandomHorizontalOffset(
                                 (1f - (float)thrower.skills.GetSkill(SkillDefOf.Shooting).level / 20f) * 1.8f);
            ...
            moteThrown.airTimeLeft =
                (float)Mathf.RoundToInt((moteThrown.exactPosition - vector).MagnitudeHorizontal() / num);
        ...
        {
            if (!cell.ShouldSpawnMotesAt() || MoteCounterTFH.Saturated)
            {
                return null;
            }
