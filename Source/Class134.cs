...
namespace too
{
    public class HediffGiver_Bomb : HediffGiver
    {
        ...
        {
            HediffGiver_Bomb.addedHediffs.Clear();
            if (!base.TryApply(pawn, HediffGiver_Bomb.addedHediffs))
            {
                return;
            }
            if (this.averageSeverityPerDayBeforeGeneration != 0f)
            {
                ...
                for (int i = 0; i < HediffGiver_Bomb.addedHediffs.Count; i++)
                {
                    this.SimulateSeverityChange(pawn, HediffGiver_Bomb.addedHediffs[i], num, tryNotToKillPawn);
                }
            }
            HediffGiver_Bomb.addedHediffs.Clear();
        }
